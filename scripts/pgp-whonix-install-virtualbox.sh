#!/usr/bin/env bash
#
# pgp-whonix-install-virtualbox.sh — OpenPGP-GPG-do-Zero-ao-Expert (host Linux) - port da suite Privacy-OS-Hub whonix-host v3.5.4
#
# Passo 10 — pré-requisito: Oracle VirtualBox verificado no Debian/Ubuntu.
# Playbook: playbooks/10-whonix-capstone.md
#
# Uso:
#   sudo ./pgp-whonix-install-virtualbox.sh [-v VERSAO] [-y] [--no-extpack] [--skip-mok]
...
#   3 — pacote OK; falta pgp-whonix-sign-virtualbox-modules.sh
#   1 — erro fatal
#
# Log: /var/log/virtualbox-install.log (linha RESULTADO: no final)
# Assinatura de módulos: pgp-whonix-sign-virtualbox-modules.sh (log separado)
#
...
#   - Assinatura separada: pgp-whonix-sign-virtualbox-modules.sh
...
SIGN_SCRIPT="${SCRIPT_DIR}/pgp-whonix-sign-virtualbox-modules.sh"
...
invoke_sign_script() {
    ...
    if [[ ! -x "$SIGN_SCRIPT" ]]; then
        warn "pgp-whonix-sign-virtualbox-modules.sh ausente em ${SIGN_SCRIPT}"
        warn "Rode: sudo ./pgp-whonix-sign-virtualbox-modules.sh -y --qa-log"
        return 1
    fi
    log "Delegando assinatura → pgp-whonix-sign-virtualbox-modules.sh"
    set +e
    "$SIGN_SCRIPT" -y 2>&1 | tee -a "$LOG_FILE" >&2
    sign_rc=${PIPESTATUS[0]}
    set -e
    if [[ "$sign_rc" -eq 0 ]]; then
        log "Assinatura OK (pgp-whonix-sign-virtualbox-modules.sh)."
        return 0
...
print_wizard_intro() {
    ...
    case "$WIZARD_PHASE" in
        ...
        pending_mok_reboot)
            ...
            ;;
        needs_sign)
            phase_msg="Fase: ASSINAR MÓDULOS — MOK OK ou SB off; rode pgp-whonix-sign-virtualbox-modules.sh."
            ;;
        installed_need_mok)
    ...
    _m ""
    _m "==================================================================="
    _m "  Assistente VirtualBox — OpenPGP-GPG-do-Zero-ao-Expert · v3.5.4"
    _m "==================================================================="
    _b "  ${phase_msg}"
    echo "" >&2
    ...
    if [[ "$WIZARD_PHASE" == "pending_mok_reboot" ]]; then
        ...
        _y "  Próximo: sudo ./pgp-whonix-sign-virtualbox-modules.sh -y --qa-log"
    fi
    echo "" >&2
    _b "  Validar: ./pgp-whonix-verify-virtualbox-host.sh --qa-log"
...
===================================================================
  ...
  3) De volta ao Debian (ordem recomendada):
       ...
       sudo ./pgp-whonix-verify-virtualbox-host.sh --qa-log
       sudo ./pgp-whonix-install-virtualbox.sh -y    # Extension Pack se faltar
  ...
  · Refaça do zero:
      sudo ./pgp-whonix-install-virtualbox.sh --reset-mok --new-mok-keys -y
...
handle_secure_boot_modules() {
    log "[Passo 8/11] Módulos kernel — delegado ao pgp-whonix-sign-virtualbox-modules.sh"
...
print_summary() {
    local next_action="Relogin (vboxusers) → pgp-whonix-import-ova.sh"
    echo "" >&2
    log "===== Resumo [11/11] ====="
    if vbox_modules_loaded; then
        log "Módulos kernel: OK (vboxdrv carregado)"
        next_action="sudo ./pgp-whonix-verify-virtualbox-host.sh --qa-log → pgp-whonix-import-ova.sh"
    elif [[ "$MOK_REBOOT_NEEDED" -eq 1 ]]; then
        log "Módulos kernel: PENDENTE_REBOOT_MOK"
        next_action="sudo systemctl reboot -i → Enroll MOK → pgp-whonix-sign-virtualbox-modules.sh -y"
    elif virtualbox_pkg_installed; then
        log "Módulos kernel: PENDENTE_SIGN"
        next_action="sudo ./pgp-whonix-sign-virtualbox-modules.sh -y --qa-log"
    else
